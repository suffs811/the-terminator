...
# <> purpose: create a .docx file of the report.txt generated by terminator.py
#
# usage: python3 doc.py (optional: -r <path_to_report_file> (if name was changed from /terminator/report.txt))
...
parser = argparse.ArgumentParser(description="create a .docx file of the report.txt generated by terminator.py\nusage: python3 doc.py (optional: -r <path_to_report.txt> (if name was changed from report.txt))")
parser.add_argument("-r", "--report", help="specify path to report file (default is /terminator/report.txt)")
...
def doc_make(report):
	...
	# check for inputted report path, else use default /terminator/report.txt
	report = ""
	if args.report is not None:
		report = args.report
	else:
		report = "/terminator/report.txt"
	...
	document = Document()
   	...
   	os.system("mv report.docx /terminator/report.docx")
   	...
   	print("-+- Word document saved to /terminator/report.docx -+-")
